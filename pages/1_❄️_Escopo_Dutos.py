...
with tab1:
    ...
    with c1:
        ...
        if not forn: forn = "PROPONENTE DUTOS"
...
with tab2:
    ...
    # ORDEM ALFABÉTICA
    itens_tec = st.multiselect("Itens Técnicos:", sorted(st.session_state['opcoes_db'].get('tecnico', [])), key=key_tec)
    
    c_add, c_free = st.columns(2)
    c_add.text_input("Novo T.", key="n_tec"); c_add.button("Salvar T.", on_click=adicionar_item_callback, args=("tecnico", "n_tec"))
    ...
    # ORDEM ALFABÉTICA
    itens_qual = st.multiselect("Itens Qualidade:", sorted(st.session_state['opcoes_db'].get('qualidade', [])), key=key_qual)
    
    c_q1, c_q2 = st.columns(2)
    c_q1.text_input("Novo Q.", key="n_qual"); c_q1.button("Salvar Q.", on_click=adicionar_item_callback, args=("qualidade", "n_qual"))
...
with tab5:
    val = st.text_input("Valor Total (Ex: 15000.00)", dados_edicao.get('Valor', '')); pgto = st.text_area("Pagamento"); info = st.text_input("Info"); obs = st.text_area("Obs")
    st.divider()
