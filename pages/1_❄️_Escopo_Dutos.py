...
def gerar_docx(dados):
    ...
    pf = footer.add_paragraph(); pf.text = "SIARCON - Controle Ambiental"; pf.alignment = 1; pf.style.font.size = Pt(9); pf.style.font.italic = True
...
with tab2:
    ...
    itens_tec = st.multiselect("Itens Técnicos:", st.session_state['opcoes_db'].get('tecnico', []), key=key_tec)
    ...
    itens_qual = st.multiselect("Itens Qualidade:", st.session_state['opcoes_db'].get('qualidade', []), key=key_qual)
    
    c_q1, c_q2 = st.columns(2)
    c_q1.text_input("Novo Item Q.", key="n_qual"); c_q1.button("Salvar Q.", on_click=adicionar_item_callback, args=("qualidade", "n_qual"))
