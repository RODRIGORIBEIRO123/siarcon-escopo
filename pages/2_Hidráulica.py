...
PADRAO_TECNICO = [
    "Montagem de Tubulação em Aço Carbono (Solda/Rosca)", "Fabricação e montagem da suportação",
    "Montagem de Tubulação em PPR/PVC", "Fabricação e montagem dos cavaletes de água gelada",
    "Fabricação e montagem dos cavaletes de água quente", "Fabricação e montagem dos cavaletes de vapor e condensado",
    "Fabricação e montagem dos cavaletes das bombas", "Fabricação e montagem dos cavaletes dos chillers",
    "Fabricação e montagem dos cavaletes do trocador de calor", "Instalação de Válvulas de Controle e Bloqueio",
    "Instalação de Válvulas de Balanceamento", "Instalação de Filtros Y e Purgadores",
    "Instalação de Manômetros e Termômetros", "Aplicação de Isolamento em Borracha Elastomérica",
    "Aplicação de isolamento em calhas de lã de rocha", "Aplicação de isolamento nas redes de dreno",
    "Montagem da rede de dreno", "Proteção Mecânica em Alumínio liso", 
    "Proteção Mecânica em Alumínio corrugado", "Instalação de Bombas", "Execução de Drenos de Condensados"
]

PADRAO_QUALIDADE = [
    "Teste Hidrostático de Pressão", "Inspeção Visual de Soldas", 
    "Aplicação de fundo das tubulações", "Verificação de Alinhamento e Prumo", 
    "Limpeza da rede de água", "Limpeza dos filtro Y", "Limpeza do local de trabalho"
]
...
with tab2:
    ...
    if c_a2.button("💾 Criar Téc."): 
        if novo_item and utils_db.aprender_novo_item(cat_tecnica_db, novo_item): 
            ...
            lista_tec_final.append(novo_item); lista_tec_final = sorted(list(set(lista_tec_final)))
    ...
    comentarios_novos = {}
    if itens_tec:
        st.caption("📝 Detalhe os itens:")
        for i in itens_tec: comentarios_novos[i] = st.text_input(f"Detalhe '{i}':", value=comentarios_salvos.get(i, ""))
    ...
    if c_q2.button("💾 Criar Qual."): 
        if novo_item_q and utils_db.aprender_novo_item(f"qualidade_{DISCIPLINA_ATUAL.lower()}", novo_item_q): 
            st.session_state['opcoes_db'] = utils_db.carregar_opcoes()
            lista_qual.append(novo_item_q); lista_qual = sorted(list(set(lista_qual)))
            st.toast("✅ Qualidade salva no banco!")
