...
def gerar_docx(dados):
    ...
    document.add_heading('Escopo de fornecimento - Rede Hidráulica', 0).alignment = 1
    ...
    # Docs Fixos apenas
    docs = ["Ficha de registro", "ASO (Atestado Saúde Ocupacional)", "Ficha de EPI", "Ordem de Serviço", "Certificados Treinamento"]
...
with tab1:
    ...
    with c1:
        ...
        if not forn: forn = "PROPONENTE HIDRÁULICA"
...
with tab2:
    ...
    lista_tec = sorted(st.session_state['opcoes_db'].get('tecnico_hidraulica', []))
    itens_tec = st.multiselect("Itens Técnicos:", options=lista_tec, key=key_tec)
    
    c_add, c_free = st.columns(2)
    c_add.text_input("Novo Item T.", key="n_tec"); c_add.button("Salvar T.", on_click=adicionar_item_callback, args=("tecnico_hidraulica", "n_tec"))
    ...
    lista_qual = sorted(st.session_state['opcoes_db'].get('qualidade_hidraulica', []))
    itens_qual = st.multiselect("Itens Qualidade:", options=lista_qual, key=key_qual)
    
    c_q1, c_q2 = st.columns(2)
    c_q1.text_input("Novo Item Q.", key="n_qual"); c_q1.button("Salvar Q.", on_click=adicionar_item_callback, args=("qualidade_hidraulica", "n_qual"))
    qual_livre = c_q2.text_input("Livre Q.")

with tab3:
    escolhas = {}
    itens_m = ["Materiais de tubulação e conexões", "Materiais de isolamento térmico", "Insumos de soldagem", "Consumíveis (Discos, eletrodos, brocas, etc)", "Materiais dos cavaletes", "Materiais dos drenos", "Válvulas de controle e balanceamento", "Plataformas elevatórias e/ou andaimes", "Ferramentas manuais e bancadas", "Escadas tipo \"A\"", "Alimentação, viagem, hospedagem", "Epis", "Uniformes"]
