...
def gerar_docx(dados):
    ...
    title = document.add_heading('Escopo de fornecimento - Instalações Elétricas', 0)
    ...
    # Lista padrão com NR-10 incluído
    docs_padrao = [
        "Ficha de registro", 
        "ASO (Atestado de Saúde Ocupacional)", 
        "NR-10 (Segurança em Instalações e Serviços em Eletricidade)", # <--- ADICIONADO
        "Ficha de EPI", 
        "Ordem de Serviço", 
...
with tab1:
    ...
    with c1:
        ...
        if not fornecedor_input: fornecedor_final = "PROPONENTE DE ELÉTRICA"
...
with tab2:
    st.subheader("Técnico (Elétrica)")
    # ITENS PADRÃO SUGERIDOS (Se a lista do DB estiver vazia, usa estes)
    padroes_tec = [
        "Fornecimento de materiais e mão de obra para instalação de infra de potência e automação",
        "Fornecimento de materiais e mão de obra para passagem e conexão dos cabos",
        "Fornecimento de materiais e mão de obra para interligação do painel de potência e automação",
        "Fixação e conectorização dos instrumentos dos equipamentos e dutos e equipamentos",
        "Realizar partida e verificação dos sistemas, corrigir sentido de rotação se necessário",
        "Acompanhar startup dos sistemas",
        "Acompanhar o comissionamento dos pontos da automação"
    ]
    
    opcoes_db_tec = st.session_state['opcoes_db'].get('tecnico_eletrica', [])
    # Une o que tem no banco com os padrões sugeridos (sem duplicar)
    ...
    with col_add:
        ...
        st.button("Salvar Item Técnico", on_click=adicionar_item_callback, args=("tecnico_eletrica", "input_novo_tec"))
    ...
    padroes_qual = [
        "Todos os cabos devem possuir terminais",
        "Todos os cabos devem estar identificados com anilhas",
        "Realizar proteção nos paineis, para cortes e furos",
        "Realizar limpeza na parte interna dos painéis"
    ]
    
    opcoes_db_qual = st.session_state['opcoes_db'].get('qualidade_eletrica', [])
    ...
    with c_q1:
        ...
        st.button("Salvar Qualidade", on_click=adicionar_item_callback, args=("qualidade_eletrica", "input_novo_qual"))
...
with tab3:
    ...
    # ITENS SOLICITADOS DE ELÉTRICA
    itens_matriz = [
        "Materiais de infraestrutura (eletrocalhas, eletrodutos, etc)",
        "Cabeamento",
        "Consumíveis (Fita isolante, brocas, discos de corte)",
...
with tab4:
    ...
    nrs = st.multiselect("SMS Adicional (NR-10 já incluso):", options=opcoes_sms)
...
if status_atual == "Contratação Finalizada" and 'modo_edicao' in st.session_state:
    ...
    if st.button(label_botao, type="primary", use_container_width=True):
        ...
        if novo_status == "Contratação Finalizada":
            if fornecedor_final == "PROPONENTE DE ELÉTRICA" or not fornecedor_final.strip():
        ...
        if not erro:
            ...
            dados = {
                ...
                'status': novo_status,
                'disciplina': 'Elétrica', # <--- FORÇA DISCIPLINA
