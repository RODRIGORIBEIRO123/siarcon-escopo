...
def gerar_docx(dados):
    ...
    for d in ["Ficha Registro", "ASO", "Ficha EPI", "NR-11"] + dados.get('nrs_selecionadas', []): document.add_paragraph(d, style='List Bullet')
...
with tab5:
    val = st.text_input("Total", dados_edicao.get('Valor', '')); pgto = st.text_area("Pgto"); info = st.text_input("Info"); obs = st.text_area("Obs")
    status = st.selectbox("Status", ["Em Elaboração (Engenharia)", "Aguardando Obras", "Recebido (Suprimentos)", "Enviado para Cotação", "Contratação Finalizada"])
