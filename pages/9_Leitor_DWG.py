...
st.title("📐 Leitor Técnico DXF - Filtro Inteligente de Terminais")
st.markdown("""
**Correção Aplicada:** Itens com medidas terminadas em **25** (ex: 425x125, 625x625) são automaticamente classificados como **Terminais/Grelhas** e removidos do cálculo de dutos.
""")
...
with st.sidebar:
    ...
    st.info("ℹ️ Raio de Busca: Distância do texto até as linhas do duto.")
    raio_busca = st.number_input("Raio de Busca (Unidades CAD)", value=2.0)
...
def extrair_todos_textos(msp):
    """Extrai textos de TEXT, MTEXT e ATRIBUTOS (Blocos)."""
    lista_textos = []
    ...
    # Blocos com Atributos (Tags de Grelhas geralmente são assim)
...
def limpar_e_parsear(texto_raw):
    ...
    if match:
        try:
            ...
            # Remove ponto de milhar (ex: 1.300 -> 1300)
...
def medir_duto(msp, texto_obj, w_target, h_target, layers, raio):
    ...
    for i in range(len(candidatos)):
        ...
        for j in range(i+1, len(candidatos)):
            ...
            for esc in escalas:
                w_t, h_t = w_target * esc, h_target * esc
...
def processar(doc, layers_duto, raio, padrao):
    msp = doc.modelspace()
    dutos = []
    restos = [] # Vai pra IA (Terminais, Equipamentos)
    logs = []
    ...
    for item in todos_textos:
        ...
        if largura:
            # --- FILTRO DE GRELHAS (Termina com 25) ---
            # Ex: 425, 125, 625...
            eh_grelha = (str(int(largura)).endswith('25') or str(int(altura)).endswith('25'))
            
            if eh_grelha:
                # Se for grelha, NÃO mede como duto. Manda pra IA classificar como Terminal.
                restos.append(t_limpo)
                logs.append(f"💨 GRELHA DETECTADA: {t_limpo} (Movido para Terminais)")
            else:
                # É Duto -> Tenta Medir
                ...
                logs.append(f"✅ DUTO: {t_limpo} -> {val_final:.2f}m ({status})")
        else:
            # Se não tem dimensão clara, mas tem texto, manda pra IA
...
def ia_class(lista):
    ...
    c = Counter(lista)
    # Aumentei o contexto para pegar mais grelhas se houver muitas
    p = "\n".join([f"{k} (x{v})" for k,v in c.most_common(300)])
    
    sys = """
    Analise a lista de itens HVAC.
    Regra Principal: Medidas terminadas em 25 (ex: 425x125) SÃO GRELHAS.
...
if uploaded_dxf:
    ...
    if erro:
        ...
        if st.button("🚀 Processar", type="primary"):
            with st.spinner("Classificando Dutos vs Grelhas..."):
...
if 'res_dutos' in st.session_state:
    ...
    with t1:
        if dutos:
            ...
            k1, k2 = st.columns(2)
            k1.metric("Trechos de Duto", len(df))
            ...
            df_ed = st.data_editor(
                ...
                column_config={"Largura": st.column_config.NumberColumn(format="%d mm"),
            ...
            st.warning("Nenhum duto encontrado (após filtro de grelhas).")

    with t2:
        st.markdown("### 💨 Grelhas e Difusores (Filtro '25')")
        if ia.get("TERMINAIS"): st.data_editor(pd.DataFrame(ia["TERMINAIS"], columns=["Item","Qtd"]), use_container_width=True)
        else: st.info("Nenhuma grelha identificada.")
